	...
	return &config{
		app: &app{
			...
			writeTimeout: func() time.Duration {
				...
				if err != nil {
					log.Fatalf("failed to convert write time out: %v", err)
				}
				return time.Duration(int64(t) * int64(math.Pow10(9)))
			}(), bodyLimit: func() int {
				b, err := strconv.Atoi(envMap["APP_BODY_LIMIT"])
				if err != nil {
					log.Fatalf("failed to convert  body limit: %v", err)
				}
				return b
			}(),
			fileLimit: func() int {
				...
				if err != nil {
					log.Fatalf("failed to convert  file limit: %v", err)
				}
				return f
			}(),
		...
		db: &db{
			...
			port: func() int {
				...
				if err != nil {
					log.Fatalf("failed to convert  db  port: %v", err)
				}
				return p
			}(),
